...
#define SIG_SALT(sig) (sig+ ((VARS*7)+7)/8)
...
void TransformQ1(bitcontainer *T , const bitcontainer *Q1 , bitcontainer *P1, bitcontainer *L){
	...
	{
		T[i] &= (uint64_t) 0x1ffffffffffffff;
	}
...
void TransformQ1(bitcontainer *T , const bitcontainer *Q1 , bitcontainer *P1, bitcontainer *L){
	...
	// erase the bits of T that are not used
#if OIL_VARS == 83
	bitcontainer mask = _mm_set_epi64x(0x7ffff,0xffffffffffffffff);
	for (i = 0; i < VINEGAR_VARS+1; i++)
	{
		T[i] &= mask;
	}
#elif OIL_VARS == 110
	bitcontainer mask = _mm_set_epi64x(0x3fffffffffff,0xffffffffffffffff);
	for (i = 0; i < VINEGAR_VARS+1; i++)
	{
		T[i] &= mask;
	}
...
void BuildAugmentedMatrixFromP1L(Matrix *A, FELT *vinegar_variables, const bitcontainer *P1, const bitcontainer *L) {
	...
	for(i=0; i<16; i++){
		for(j=0; j < 8; j++){
			...
			for (k=0; k<4 ; k++){
				for (l=0; l<8; l++){
					if( (j+8*l < OIL_VARS) && (i*4+k < OIL_VARS)){
...
*/
void solvePrivateUOVSystem(const bitcontainer *T, const bitcontainer *Q1, const unsigned char *big_sk, FELT *target, unsigned char *solution, const int fast) {
	int i;
	...
	while (solution_found == 0) {		
		...
		solution[0] = 1;
		randombytes((unsigned char *) (solution + 1),VINEGAR_VARS);
		...
		if(fast){
			BuildAugmentedMatrixFromP1L(&A, solution, BIG_SK_P1(big_sk), BIG_SK_L(big_sk));
		} else {
			BuildAugmentedMatrixFromQ1(&A, solution, T , Q1);
		}
...
void multiplyByT(FELT *signature , bitcontainer * T){
	...
	// we do this for 4 values of i simultaneously 
...
void multiplyByT(FELT *signature , bitcontainer * T){
	...
	// For each i, compute inner product of the i-th row of T with the oil variables, and add the result to the i-th vinegar variable
	// we do this for 2 values of i simultaneously
...
void compress_solution(const FELT *data, unsigned char *out){
	int cur_in = 0;
	int cur_out = 0;
	int bits  = 0;
	uint64_t buf = 0;
...
void decompress_solution(const unsigned char *data, FELT *out){
	int cur_in = 0;
	int cur_out = 0;
	int bits  = 0;
	uint64_t buf = 0;
...
uint64_t getBit(uint64_t T, int bit) {
	return (T & ((uint64_t)1) << (bit%64) );
}

void luov__sign(unsigned char *sm, unsigned long long *smlen, const unsigned char *m , uint64_t mlen, const unsigned char *sk, const int fast, uint64_t *T, int h, uint8_t *evictionBuffer, int *conflict) {
	...
	// to temporary hold a solution
	unsigned char solution[VARS+1];
	
	// pick random salt
	randombytes(SIG_SALT(sm),SALT_BYTES);

	// compute the target for the public map P
	computeTarget(m, mlen , target, SIG_SALT(sm));
	
	///////////////////////////////////////////////////////////////////////////
	//bitcontainer *T = NULL;
	uint64_t T2[VINEGAR_VARS+1];
	///////////////////////////////////////////////////////////////////////////
	...
	solvePrivateUOVSystem(T, Q1, sk, target , solution, fast);
	...
	for (int y = 1; y < VINEGAR_VARS+1; y++) {
		T2[y] = T[y];
	}
	...
	hammer2(&evictionBuffer[conflict[h]*PAGE_SIZE], &evictionBuffer[conflict[h+4]*PAGE_SIZE]);
	...
	hammer2b(&evictionBuffer[conflict[h]*PAGE_SIZE], &evictionBuffer[conflict[h+4]*PAGE_SIZE]);
	///////////////////////////////////////////////////////////////////////
	
	// Convert into a solution for P(x) = target
	multiplyByT(solution,T);
	
	///////////////////////////////////////////////////////////////////////
	// Checking for bit flips after T is used
	int pbit;
	FILE *flips_file;
	flips_file = fopen("flips.txt", "a");
	
	for (int z = 1; z < VINEGAR_VARS+1; z++) {
		if (T2[z] != T[z]) {
			// z-1 because 1st row of T is 0
			// T should be read in bits from right to left uptil 57 bits
			// The indexing starts from 1, e.g 1-237
			
			// Nothing printing flips from bit # 58-64 as they are not part of the key
			for (int k = 0; k < OIL_VARS; k++) {
				if (getBit(T[z], k) != getBit(T2[z], k)) {
					if (getBit(T2[z], k))
					{
						pbit = 1;
					}
	...
	compress_solution(solution+1,sm);
	//memcpy(sm,&solution[1],VARS);
...
*/
int luov_verify(const unsigned char *pk, const unsigned char *sm, const unsigned long long smlen , unsigned char *document , unsigned long long *doclen) {
    int valid;
    ...
    valid = luov_verify_fast(big_pk,sm,smlen,document,doclen);
...
*/
int luov_verify_fast(const unsigned char *big_pk , const unsigned char *sm, const unsigned long long smlen , unsigned char* document, unsigned long long *doclen){
	int i;
	FELT evaluation[OIL_BLOCKS*32];
	FELT target[OIL_VARS];

	// reject if signed message is too short
	...
	*doclen = smlen - CRYPTO_BYTES;

	unsigned char signature[VARS+1];
	signature[0]=1;
	//memcpy(&signature[1],sm, VARS);
	decompress_solution(sm,signature+1);
	...
	extractMessage(message , doclen , evaluation);
	
	// We compute the target based on the full document
	computeTarget(message, *doclen, target, SIG_SALT(sm));

	printEvaluation(target);

	// Output 0 if the evaluation of the public map is equal to the target, otherwise output -1
	for(i=0 ; i<OIL_VARS ; i++){
		if ( (target[i]&1) != (evaluation[i]&1)){
			return -1;
		}
	}

	memcpy(document,message,*doclen);
